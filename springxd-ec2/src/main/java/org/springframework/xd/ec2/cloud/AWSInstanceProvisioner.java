...
public class AWSInstanceProvisioner implements InstanceProvisioner {
	...
	private static String theRegionString;
	...
	public Reservation<? extends RunningInstance> runInstance(String script,
		...
		RunInstancesOptions	instancesOptions = asType(getNewInstanceType());
		instancesOptions = instancesOptions.withKeyName(publicKeyName);
		...
		Reservation<? extends RunningInstance> reservation = instanceApi.
...
//								.withUserData(script.getBytes()));
	...
	public static RunningInstance findInstanceById(AWSEC2Api client,
		...
		Optional<? extends AWSInstanceApi> optionalInstanceApi = client.getInstanceApi();
		AWSInstanceApi instanceApi = optionalInstanceApi.get();
		Set<? extends Reservation<? extends RunningInstance>> reservations = instanceApi.describeInstancesInRegion(theRegionString, instanceId);
		
		RunningInstance runningInstance = Iterables.getOnlyElement(Iterables.getOnlyElement(reservations));
...
//		return Iterables.getOnlyElement(Iterables.getOnlyElement(reservations));
	...
	private String getNewInstanceType() {
		String type = null;
		if (machineSize.equalsIgnoreCase(InstanceSize.SMALL.name())) {
			type = "t2.micro";
		}
